...
uses
  ...
  TntSysUtils, gnugettext, RegExpr, KKTDrvLib_TLB;

type
  { TFiscalPrinterDriverRR }
  ...
  public
    property Driver: TDriverRR read FDriver;
...
constructor TFiscalPrinterDriverRR.Create(AContext: TDriverContext);
begin
  inherited Create;
  FContext := AContext;
  ...
  FLogger := TClassLogger.Create('TFiscalPrinterDriverRR', FContext.Logger);
...
function TFiscalPrinterDriverRR.GetDepartment(ADepartment: Integer): Integer;
begin
  Result := ADepartment;
end;

function TFiscalPrinterDriverRR.IntToAmount(Value: Int64): Currency;
begin
  Result := Value / 100;
end;

function TFiscalPrinterDriverRR.IntToQuantity(Value: Int64): Double;
begin
  Result := Value / 1000;
end;

function TFiscalPrinterDriverRR.AmountToInt(Value: Currency): Int64;
begin
...
procedure TFiscalPrinterDriverRR.CheckDriver(Code: Integer);
const
  E_NOHARDWARE = -1;  //  
  E_NOPORT = -2;  // OM  
  E_PORTBUSY = -3;  // OM    
...
procedure TFiscalPrinterDriverRR.SetDriverPassword(Password: DWORD);
begin
  //Driver.Password := Password;
end;

procedure TFiscalPrinterDriverRR.Connect;
begin
  GetDeviceMetrics;
end;
...
procedure TFiscalPrinterDriverRR.MapShortStatusFromDriver(
...
procedure TFiscalPrinterDriverRR.MapLongStatusFromDriver(
...
procedure TFiscalPrinterDriverRR.Initialize;
begin
  Tables.Clear;
  Fields.Clear;
...
function TFiscalPrinterDriverRR.GetCapSubtotalRound: Boolean;
begin
  Result := FCapSubtotalRound;
end;

function TFiscalPrinterDriverRR.GetCapDiscount: Boolean;
begin
  Result := FCapDiscount;
end;

function TFiscalPrinterDriverRR.GetParameters: TPrinterParameters;
begin
  Result := FContext.Parameters;
end;

function TFiscalPrinterDriverRR.GetLogger: ILogFile;
begin
  Result := FContext.Logger;
end;

function TFiscalPrinterDriverRR.GetMalinaParams: TMalinaParams;
begin
  Result := FContext.MalinaParams;
end;

function TFiscalPrinterDriverRR.GetCapReceiptDiscount: Boolean;
begin
  Result := FCapReceiptDiscount;
end;

procedure TFiscalPrinterDriverRR.AddFilter(AFilter: IFiscalPrinterFilter);
begin
  FFilter.AddFilter(AFilter);
end;

procedure TFiscalPrinterDriverRR.RemoveFilter(AFilter: IFiscalPrinterFilter);
begin
  FFilter.RemoveFilter(AFilter);
end;

function TFiscalPrinterDriverRR.GetResultCode: Integer;
begin
  Result := FResultCode;
end;

function TFiscalPrinterDriverRR.GetResultText: WideString;
begin
  Result := FResultText;
end;

function TFiscalPrinterDriverRR.GetStatistics: TFiscalPrinterStatistics;
begin
  Result := FStatistics;
end;

procedure TFiscalPrinterDriverRR.Lock;
begin
  FLock.Enter;
end;

procedure TFiscalPrinterDriverRR.Unlock;
begin
  FLock.Leave;
end;

function TFiscalPrinterDriverRR.GetModelsFileName: WideString;
begin
  Result := IncludeTrailingBackSlash(ExtractFilePath(GetDllFileName)) +
      ModelsFileName;
end;

procedure TFiscalPrinterDriverRR.LoadModels;
var
...
begin
  ...
  try
    ...
    on E: Exception do
      Logger.Error('TFiscalPrinterDriverRR.LoadModels', E);
  end;
  Reader.Free;
end;

procedure TFiscalPrinterDriverRR.ReadModelData;
begin
  ReadModelTables;
  ReadModelParameters;
end;

procedure TFiscalPrinterDriverRR.ReadModelTables;
var
...
procedure TFiscalPrinterDriverRR.ReadModelParameters;
var
  Text: AnsiString;
  ParameterID: Integer;
  FieldValue: AnsiString;
...
procedure TFiscalPrinterDriverRR.SaveModels;
var
  Reader: TXmlModelReader;
begin
  Reader := TXmlModelReader.Create(FModels, Logger);
...
function TFiscalPrinterDriverRR.GetLine(const Text: WideString): WideString;
begin
  Result := GetLine(Text, MinLineWidth, GetPrintWidth);
end;

function TFiscalPrinterDriverRR.GetLine(const Text: WideString;
...
function TFiscalPrinterDriverRR.GetText(const Text: WideString;
...
function TFiscalPrinterDriverRR.GetPrintWidth: Integer;
begin
  Result := GetPrintWidth(Parameters.FontNumber);
end;

function TFiscalPrinterDriverRR.ValidFont(Font: Integer): Boolean;
begin
  Result := (Font >= 1) and (Font <= Length(FFontInfo));
end;

function TFiscalPrinterDriverRR.GetPrintWidth(Font: Integer): Integer;
begin
  Result := 0;
...
function TFiscalPrinterDriverRR.GetSysPassword: DWORD;
begin
  Result := FSysPassword;
end;

function TFiscalPrinterDriverRR.GetTaxPassword: DWORD;
begin
  Result := FTaxPassword;
end;

function TFiscalPrinterDriverRR.GetUsrPassword: DWORD;
begin
  Result := FUsrPassword;
end;

procedure TFiscalPrinterDriverRR.SetSysPassword(const Value: DWORD);
begin
  FSysPassword := Value;
end;

procedure TFiscalPrinterDriverRR.SetTaxPassword(const Value: DWORD);
begin
  FTaxPassword := Value;
end;

procedure TFiscalPrinterDriverRR.SetUsrPassword(const Value: DWORD);
begin
  FUsrPassword := Value;
end;

function TFiscalPrinterDriverRR.ReadFieldStructure(Table, Field: Byte): TPrinterFieldRec;
var
...
function TFiscalPrinterDriverRR.ValidFieldValue(
...
function TFiscalPrinterDriverRR.ReadTableStructure(Table: Byte;
...
class function TFiscalPrinterDriverRR.BaudRateToCode(BaudRate: Integer): Integer;
begin
  case BaudRate of
    CBR_2400    : Result := 0;
    CBR_4800    : Result := 1;
...
class function TFiscalPrinterDriverRR.CodeToBaudRate(BaudRate: Integer): Integer;
begin
  case BaudRate of
    0: Result := CBR_2400;
    1: Result := CBR_4800;
...
class function TFiscalPrinterDriverRR.ByteToTimeout(Value: Byte): DWORD;
begin
  case Value of
    0..150   : Result := Value;
...
class function TFiscalPrinterDriverRR.TimeoutToByte(Value: Integer): Byte;
begin
  case Value of
    0..150        : Result := Value;
...
procedure TFiscalPrinterDriverRR.SetIsOnline(Value: Boolean);
begin
...
function TFiscalPrinterDriverRR.GetErrorText(Code: Integer): WideString;
begin
  Result := PrinterTypes.GetErrorText(Code, FCapFiscalStorage);
end;

function TFiscalPrinterDriverRR.ExecuteData(const TxData: AnsiString;
...
function TFiscalPrinterDriverRR.ExecuteStream(Stream: TBinStream): Integer;
var
  RxData: AnsiString;
  TxData: AnsiString;
begin
...
function TFiscalPrinterDriverRR.ExecuteStream2(Stream: TBinStream): Integer;
var
  RxData: AnsiString;
  TxData: AnsiString;
begin
...
procedure TFiscalPrinterDriverRR.CashIn(Amount: Int64);
begin
...
procedure TFiscalPrinterDriverRR.CashOut(Amount: Int64);
begin
...
function TFiscalPrinterDriverRR.StartDump(DeviceCode: Integer): Integer;
begin
  Driver.DeviceCode := DeviceCode;
  Result := Driver.DampRequest;
end;

function TFiscalPrinterDriverRR.GetDumpBlock: TDumpBlock;
begin
...
procedure TFiscalPrinterDriverRR.StopDump;
begin
end;

function TFiscalPrinterDriverRR.LongFisc(NewPassword: DWORD;
...
procedure TFiscalPrinterDriverRR.SetLongSerial(Serial: Int64);
begin
...
function TFiscalPrinterDriverRR.ReadOperatorNumber(Password: Integer): Integer;
begin
  Result := ReadShortStatus2(Password).OperatorNumber;
end;

function TFiscalPrinterDriverRR.ReadUsrOperatorNumber: Integer;
begin
  Result := ReadShortStatus2(GetUsrPassword).OperatorNumber;
end;

function TFiscalPrinterDriverRR.ReadSysOperatorNumber: Integer;
begin
  Result := ReadShortStatus2(GetSysPassword).OperatorNumber;
end;

function TFiscalPrinterDriverRR.ReadShortStatus2(Password: Integer): TShortPrinterStatus;
var
...
function TFiscalPrinterDriverRR.ReadShortStatus: TShortPrinterStatus;
var
  Status: TPrinterStatus;
begin
  DoConnect;
...
function TFiscalPrinterDriverRR.ReadLongStatus: TLongPrinterStatus;
var
  Status: TPrinterStatus;
begin
  DoConnect;
...
function TFiscalPrinterDriverRR.GetFMFlags(Flags: Byte): TFMFlags;
begin
...
function TFiscalPrinterDriverRR.PrintBoldString(Flags: Byte; const Text: WideString): Integer;
begin
...
function TFiscalPrinterDriverRR.Beep: Integer;
begin
  Result := Driver.Beep;
end;

function TFiscalPrinterDriverRR.SetPortParams(Port: Byte;
...
function TFiscalPrinterDriverRR.GetPortParams(Port: Byte): TPortParams;
begin
  Driver.PortNumber := Port;
...
procedure TFiscalPrinterDriverRR.ResetFiscalMemory;
begin
  Driver.Check(Driver.ResetSettings);
end;

function TFiscalPrinterDriverRR.GetPrintFlags(Flags: Integer): Integer;
begin
  Result := Flags;
...
procedure TFiscalPrinterDriverRR.SetPrintFlags(Flags: Byte);
begin
...
procedure TFiscalPrinterDriverRR.OpenDay;
begin
...
function TFiscalPrinterDriverRR.ReadCashReg(ID: Integer; var R: TCashRegisterRec): Integer;
begin
...
function TFiscalPrinterDriverRR.ReadCashReg2(RegID: Integer): Int64;
...
function TFiscalPrinterDriverRR.ReadCashRegister(ID: Integer): Int64;
var
  R: TCashRegisterRec;
begin
...
function TFiscalPrinterDriverRR.ReadOperatingReg(ID: Byte;
...
function TFiscalPrinterDriverRR.ReadOperatingRegister(ID: Byte): Word;
var
  R: TOperRegisterRec;
begin
...
procedure TFiscalPrinterDriverRR.WriteLicense(License: Int64);
begin
  FLogger.Debug(Format('WriteLicense(%d)', [License]));
  { !!! }
end;

function TFiscalPrinterDriverRR.ReadLicense: Int64;
begin
  { !!! }
  Result := 0;
end;

function TFiscalPrinterDriverRR.DoWriteTable(
...
function TFiscalPrinterDriverRR.ReadTableBin(Table, Row,
...
procedure TFiscalPrinterDriverRR.SetPointPosition(PointPosition: Byte);
begin
...
procedure TFiscalPrinterDriverRR.WriteDate(const Date: TPrinterDate);
begin
...
procedure TFiscalPrinterDriverRR.ConfirmDate(const Date: TPrinterDate);
begin
...
procedure TFiscalPrinterDriverRR.InitializeTables;
begin
...
procedure TFiscalPrinterDriverRR.PartialCut;
begin
  CutPaper(PRINTER_CUTTYPE_PARTIAL);
end;

function TFiscalPrinterDriverRR.ReadFontInfo(FontNumber: Byte): TFontInfo;
begin
...
procedure TFiscalPrinterDriverRR.ResetTotalizers;
begin
  Driver.Check(Driver.ResetSummary);
end;

procedure TFiscalPrinterDriverRR.OpenDrawer(DrawerNumber: Byte);
begin
...
procedure TFiscalPrinterDriverRR.FeedPaper(Station: Byte; Lines: Byte);
begin
...
procedure TFiscalPrinterDriverRR.EjectSlip(Direction: Byte);
begin
  FLogger.Debug(Format('EjectSlip(%d)',
    [Direction]));
end;

procedure TFiscalPrinterDriverRR.StopTest;
begin
  Driver.Check(Driver.InterruptTest);
end;

procedure TFiscalPrinterDriverRR.PrintActnTotalizers;
begin

end;

function TFiscalPrinterDriverRR.ReadTableInfo(Table: Byte;
  var R: TPrinterTableRec): Integer;
begin
...
function TFiscalPrinterDriverRR.ReadFieldInfo(Table, Field: Byte;
...
procedure TFiscalPrinterDriverRR.PrintStringFont(Flags, Font: Byte;
  const Line: WideString);
var
  Text: AnsiString;
  Code: Integer;
...
procedure TFiscalPrinterDriverRR.PrintLines(const Line1, Line2: WideString);
begin
  PrintStringFont(PRINTER_STATION_REC, Parameters.FontNumber,
    FormatLines(Line1, Line2));
end;

procedure TFiscalPrinterDriverRR.PrintCommStatus;
var
  i: Integer;
  R: TFSCommStatus;
begin
...
function TFiscalPrinterDriverRR.BeginZReport: Integer;
begin
  Result := Driver.FNBeginCloseSession;
end;

procedure TFiscalPrinterDriverRR.PrintZReport;
var
  FSState: TFSState;
begin
...
procedure TFiscalPrinterDriverRR.PrintDepartmentsReport;
begin
  Driver.Check(Driver.PrintDepartmentReport);
end;

procedure TFiscalPrinterDriverRR.PrintTaxReport;
begin
  Driver.Check(Driver.PrintTaxReport);
end;

procedure TFiscalPrinterDriverRR.PrintHeader;
begin
  Driver.Check(Driver.PrintCliche);
end;

procedure TFiscalPrinterDriverRR.PrintDocTrailer(Flags: Byte);
begin
...
procedure TFiscalPrinterDriverRR.PrintTrailer;
begin
  Driver.Check(Driver.PrintTrailer);
end;

procedure TFiscalPrinterDriverRR.WriteSerial(Serial: DWORD);
begin
  FLogger.Debug(Format('WriteSerial(%d)', [Serial]));

end;

procedure TFiscalPrinterDriverRR.InitFiscalMemory;
begin
end;
...
function TFiscalPrinterDriverRR.ReadFMTotals(Flags: Byte; var R: TFMTotals): Integer;
begin
  FLogger.Debug(Format('ReadFMTotals(%d)', [Flags]));
  Result := 0;
end;

function TFiscalPrinterDriverRR.ReadFMLastRecordDate: TFMRecordDate;
begin

end;

function TFiscalPrinterDriverRR.ReadDaysRange: TDayRange;
begin
end;

function TFiscalPrinterDriverRR.Fiscalization(Password, PrinterID,
...
function TFiscalPrinterDriverRR.ReportOnDateRange(ReportType: Byte;
...
function TFiscalPrinterDriverRR.ReportOnNumberRange(ReportType: Byte;
...
procedure TFiscalPrinterDriverRR.InterruptReport;
begin
end;

function TFiscalPrinterDriverRR.ReadFiscInfo(FiscNumber: Byte): TFiscInfo;
begin
...
function TFiscalPrinterDriverRR.OpenSlipDoc(Params: TSlipParams): TDocResult;
begin
end;

function TFiscalPrinterDriverRR.OpenStdSlip(Params: TStdSlipParams): TDocResult;
begin
end;

function TFiscalPrinterDriverRR.SlipOperation(Params: TSlipOperation;
  Operation: TPriceReg): Integer;
begin
  Result := 0;
end;

function TFiscalPrinterDriverRR.SlipStdOperation(LineNumber: Byte;
  Operation: TPriceReg): Integer;
begin
  Result := 0;
end;

function TFiscalPrinterDriverRR.SlipDiscount(Params: TSlipDiscountParams;
  Discount: TSlipDiscount): Integer;
begin
  Result := 0;
end;

function TFiscalPrinterDriverRR.SlipStdDiscount(Discount: TSlipDiscount): Integer;
begin
  Result := 0;
end;

function TFiscalPrinterDriverRR.SlipClose(Params: TCloseReceiptParams): TCloseReceiptResult;
begin
end;

procedure TFiscalPrinterDriverRR.UpdateDepartment(var P: TPriceReg);
var
  S: AnsiString;
  V, Code: Integer;
begin
...
function TFiscalPrinterDriverRR.PrintItemText(const S: WideString): WideString;
var
  i: Integer;
  Line: AnsiString;
  Lines: TTntStrings;
...
function TFiscalPrinterDriverRR.Sale(Operation: TPriceReg): Integer;
begin
...
function TFiscalPrinterDriverRR.Buy(Operation: TPriceReg): Integer;
begin
  UpdateDepartment(Operation);
  Driver.Quantity := Operation.Quantity/1000;
  Driver.Price := IntToAmount(Operation.Price);
...
function TFiscalPrinterDriverRR.RetSale(Operation: TPriceReg): Integer;
begin
  UpdateDepartment(Operation);
  Driver.Quantity := Operation.Quantity/1000;
  Driver.Price := IntToAmount(Operation.Price);
...
function TFiscalPrinterDriverRR.RetBuy(Operation: TPriceReg): Integer;
begin
  UpdateDepartment(Operation);
  Driver.Quantity := Operation.Quantity/1000;
  Driver.Price := IntToAmount(Operation.Price);
...
function TFiscalPrinterDriverRR.Storno(Operation: TPriceReg): Integer;
begin
  UpdateDepartment(Operation);
  Driver.Quantity := Operation.Quantity/1000;
  Driver.Price := IntToAmount(Operation.Price);
...
function TFiscalPrinterDriverRR.ReceiptClose(const P: TCloseReceiptParams;
  var R: TCloseReceiptResult): Integer;
begin
  WriteTLVItems;
...
function TFiscalPrinterDriverRR.ReceiptDiscount(
...
function TFiscalPrinterDriverRR.ReceiptDiscount2(
  Operation: TReceiptDiscount2): Integer;
begin
  Result := 0;
end;

function TFiscalPrinterDriverRR.ReceiptCharge(
  Operation: TAmountOperation): Integer;
begin
  Driver.Price := IntToAmount(Operation.Amount);
  Driver.Department := GetDepartment(Operation.Department);
...
function TFiscalPrinterDriverRR.ReceiptCancel: Integer;
begin
  Result := Driver.CancelCheck;
end;

function TFiscalPrinterDriverRR.ReceiptCancelPassword(Password: Integer): Integer;
begin
  //Driver.Password := Password;
  Result := Driver.CancelCheck;
end;

procedure TFiscalPrinterDriverRR.CancelReceipt;
var
  i: Integer;
  Password: Integer;
begin
...
function TFiscalPrinterDriverRR.GetSubtotal: Int64;
begin
  Driver.Check(Driver.CheckSubTotal);
  Result := AmountToInt(Driver.Summ1);
end;

function TFiscalPrinterDriverRR.ReceiptStornoDiscount(
  Operation: TAmountOperation): Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
...
function TFiscalPrinterDriverRR.ReceiptStornoCharge(
  Operation: TAmountOperation): Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
...
function TFiscalPrinterDriverRR.PrintReceiptCopy: Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
...
const
  E_DRV_NOT_SUPPORTED = -70; // "устройство не поддерживается драйвером"
var
  Answer: AnsiString;
begin
  DoConnect;
...
function TFiscalPrinterDriverRR.ContinuePrint: Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
...
function TFiscalPrinterDriverRR.LoadGraphics1(Line: Byte; Data: AnsiString): Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
...
function TFiscalPrinterDriverRR.PrintGraphics1(Line1, Line2: Byte): Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
...
function TFiscalPrinterDriverRR.PrintBarcode(const Barcode: WideString): Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
...
function TFiscalPrinterDriverRR.PrintGraphics2(Line1, Line2: Word): Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
  Driver.FirstLineNumber := Line1;
...
function TFiscalPrinterDriverRR.LoadGraphics2(Line: Word; Data: AnsiString): Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
  Driver.LineNumber := Line;
...
function TFiscalPrinterDriverRR.PrintGraphicsLine(Height: Word; Flags: Byte;
...
function TFiscalPrinterDriverRR.ReadDeviceMetrics: TDeviceMetrics;
begin
  DoConnect;
...
function TFiscalPrinterDriverRR.FieldToInt(FieldInfo: TPrinterFieldRec;
...
function TFiscalPrinterDriverRR.FieldToStr(FieldInfo: TPrinterFieldRec;
...
function TFiscalPrinterDriverRR.BinToFieldValue(
...
function TFiscalPrinterDriverRR.GetFieldValue(FieldInfo: TPrinterFieldRec;
...
function TFiscalPrinterDriverRR.WriteTable(
  Table, Row, Field: Integer;
  const FieldValue: WideString): Integer;
var
  Data: AnsiString;
...
function TFiscalPrinterDriverRR.WriteTableInt(
  Table, Row, Field, Value: Integer): Integer;
begin
  Result := WriteTable(Table, Row, Field, IntToStr(Value));
end;

function TFiscalPrinterDriverRR.ReadTableInt(Table, Row, Field: Integer): Integer;
begin
...
function TFiscalPrinterDriverRR.ReadTableStr(Table, Row, Field: Integer): WideString;
begin
  Result := '';
  Driver.TableNumber := Table;
  Driver.RowNumber := Row;
...
function TFiscalPrinterDriverRR.GetDayDiscountTotal: Int64;
begin
  Result :=
...
function TFiscalPrinterDriverRR.GetRecDiscountTotal: Int64;
begin
  Result :=
    ReadCashRegister(64) +
    ReadCashRegister(67) -
...
function TFiscalPrinterDriverRR.GetDayItemTotal: Int64;
var
  i: Integer;
begin
  Result := 0;
...
function TFiscalPrinterDriverRR.GetRecItemTotal: Int64;
var
  i: Integer;
begin
  Result := 0;
...
function TFiscalPrinterDriverRR.GetDayItemVoidTotal: Int64;
var
  i: Integer;
begin
  Result := 0;
...
function TFiscalPrinterDriverRR.GetRecItemVoidTotal: Int64;
var
  i: Integer;
begin
  Result := 0;
...
function TFiscalPrinterDriverRR.GetEJSesssionResult(Number: Word;
...
function TFiscalPrinterDriverRR.ReadEJActivation(var Line: WideString): Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.GetEJReportLine(var Line: WideString): Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.EJReportStop: Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.DecodeEJFlags(Flags: Byte): TEJFlags;
begin
...
function TFiscalPrinterDriverRR.GetEJStatus1(var Status: TEJStatus1): Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.FormatLines(const Line1, Line2: WideString): WideString;
begin
  Result := AlignLines(Line1, Line2, GetPrintWidth);
end;

function TFiscalPrinterDriverRR.FormatBoldLines(const Line1, Line2: WideString): WideString;
begin
...
procedure TFiscalPrinterDriverRR.EJTotalsReportDate(
...
procedure TFiscalPrinterDriverRR.EJTotalsReportNumber(
...
function TFiscalPrinterDriverRR.GetModel: TPrinterModelRec;
begin
  Result := FModelData;
end;

function TFiscalPrinterDriverRR.GetPrinterModel: TPrinterModel;
begin
  if FModel = nil then
  begin
    FModel := SelectModel;
...
function TFiscalPrinterDriverRR.SelectModel: TPrinterModel;
var
  ModelID: Integer;
begin
...
function TFiscalPrinterDriverRR.GetOnCommand: TCommandEvent;
begin
  Result := FOnCommand;
end;

procedure TFiscalPrinterDriverRR.SetOnCommand(Value: TCommandEvent);
begin
  FOnCommand := Value;
end;

function TFiscalPrinterDriverRR.GetBeforeCommand: TCommandEvent;
begin
  Result := FBeforeCommand;
end;

procedure TFiscalPrinterDriverRR.SetBeforeCommand(Value: TCommandEvent);
begin
  FBeforeCommand := Value;
end;

function TFiscalPrinterDriverRR.AlignLine(const Line: WideString;
...
function TFiscalPrinterDriverRR.CenterLine(const Line: WideString): WideString;
var
  L: Integer;
  L1: Integer;
  L2: Integer;
...
function TFiscalPrinterDriverRR.ProcessLine(const Line: WideString): Boolean;
var
  Barcode: TBarcodeRec;
begin
...
procedure TFiscalPrinterDriverRR.PrintLineFont(const Data: TTextRec);
var
  i: Integer;
  Line: AnsiString;
  Lines: TTntStrings;
...
procedure TFiscalPrinterDriverRR.SplitText(const Text: WideString; Font: Integer;
  Lines: TTntStrings);
var
  Line: WideString;
  AText: WideString;
...
procedure TFiscalPrinterDriverRR.PrintTextFont(Station: Integer;
...
procedure TFiscalPrinterDriverRR.PrintText(Station: Integer; const Text: WideString);
var
  Data: TTextRec;
begin
  Data.Text := Text;
...
procedure TFiscalPrinterDriverRR.PrintText(const Data: TTextRec);
var
  i: Integer;
  Text: AnsiString;
  Line: TTextRec;
...
function TFiscalPrinterDriverRR.GetTables: TDeviceTables;
begin
  Result := FDeviceTables;
end;

procedure TFiscalPrinterDriverRR.SetTables(const Value: TDeviceTables);
begin
  FDeviceTables := Value;
end;

procedure TFiscalPrinterDriverRR.OpenPort;
begin
...
function TFiscalPrinterDriverRR.MinProtocolVersion(V1, V2: Integer): Boolean;
var
  DM: TDeviceMetrics;
begin
  DM := GetDeviceMetrics;
...
function TFiscalPrinterDriverRR.CapShortEcrStatus: Boolean;
begin
  Result := MinProtocolVersion(1, 1);
end;

function TFiscalPrinterDriverRR.CapPrintStringFont: Boolean;
begin
  Result := MinProtocolVersion(1, 1);
end;

function TFiscalPrinterDriverRR.CapGraphics: Boolean;
begin
...
procedure TFiscalPrinterDriverRR.PrintJournal(DayNumber: Integer);
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.ValidRow(Table, Row: Integer): Boolean;
var
...
function TFiscalPrinterDriverRR.ValidField(Table, Field: Integer): Boolean;
var
  TableInfo: TPrinterTableRec;
begin
  Check(ReadTableStructure(Table, TableInfo));
...
function TFiscalPrinterDriverRR.CapParameter(ParamID: Integer): Boolean;
begin
  Result := PrinterModel.Parameters.ItemByID(ParamID) <> nil;
end;

function TFiscalPrinterDriverRR.ValidParameter(const Parameter: TTableParameter): Boolean;
begin
  Result := ValidRow(Parameter.Table, Parameter.Row) and
    ValidField(Parameter.Table, Parameter.Field);
end;

procedure TFiscalPrinterDriverRR.WriteParameter(ParamID, ValueID: Integer);
var
...
function TFiscalPrinterDriverRR.ReadParameter(ParamID: Integer): Integer;
var
  Parameter: TTableParameter;
begin
  Result := 0;
...
function TFiscalPrinterDriverRR.ReadEJDocument(MACNumber: Integer;
...
function TFiscalPrinterDriverRR.ReadEJDocumentText(MACNumber: Integer): WideString;
var
  Line: WideString;
  Lines: TTntStrings;
begin
...
// 00000068 #049021
function TFiscalPrinterDriverRR.ParseEJDocument(const Text: WideString): TEJDocument;
var
  Line: WideString;
  Lines: TTntStrings;
begin
...
function TFiscalPrinterDriverRR.ReadEJActivationText(MaxCount: Integer): WideString;
var
  i: Integer;
  Line: WideString;
  Lines: TTntStrings;
...
function TFiscalPrinterDriverRR.QueryEJActivation: TEJActivation;
begin
  Result := TEJReportParser.ParseActivation(ReadEJActivationText(6));
end;

function TFiscalPrinterDriverRR.GetIsOnline: Boolean;
begin
  Result := FIsOnline;
end;

function TFiscalPrinterDriverRR.GetOnConnect: TNotifyEvent;
begin
  Result := FOnConnect;
end;

function TFiscalPrinterDriverRR.GetOnDisconnect: TNotifyEvent;
begin
  Result := FOnDisconnect;
end;

procedure TFiscalPrinterDriverRR.SetOnConnect(const Value: TNotifyEvent);
begin
  FOnConnect := Value;
end;

procedure TFiscalPrinterDriverRR.SetOnDisconnect(const Value: TNotifyEvent);
begin
  FOnDisconnect := Value;
end;

function TFiscalPrinterDriverRR.LoadGraphics(Line: Word;
...
function TFiscalPrinterDriverRR.PrintGraphics(Line1, Line2: Word): Integer;
begin
  Result := 0;
...
function TFiscalPrinterDriverRR.IsDayOpened(Mode: Integer): Boolean;
begin
  Result := (Mode and $0F) in [MODE_24NOTOVER, MODE_24OVER, MODE_REC, MODE_SLP];
end;


function TFiscalPrinterDriverRR.GetAmountDecimalPlaces: Integer;
begin
  Result := FAmountDecimalPlaces;
end;

procedure TFiscalPrinterDriverRR.SetAmountDecimalPlaces(
  const Value: Integer);
begin
  FAmountDecimalPlaces := Value;
end;

procedure TFiscalPrinterDriverRR.PrintBarcode2(const Barcode: TBarcodeRec);
...
function TFiscalPrinterDriverRR.LoadBarcodeData(BlockType: Integer;
...
function TFiscalPrinterDriverRR.PrintQRCode2D(Barcode: TBarcodeRec): Integer;
var
  Barcode2D: TBarcode2D;
begin
  Barcode.Data := Barcode.Data + #0;
...
function TFiscalPrinterDriverRR.DrawScale(const P: TDrawScale): Integer;
var
  LastLine: Integer;
begin
  LastLine := P.LastLine;
...
function TFiscalPrinterDriverRR.GetStartLine: Integer;
begin
  Result := 1;
...
function TFiscalPrinterDriverRR.Is1DBarcode(Symbology: Integer): Boolean;
begin
  Result := not Is2DBarcode(Symbology);
end;

function TFiscalPrinterDriverRR.Is2DBarcode(Symbology: Integer): Boolean;
begin
  Result := Symbology in [
    DIO_BARCODE_QRCODE,
    DIO_BARCODE_QRCODE2,
...
procedure TFiscalPrinterDriverRR.AlignBitmap(Bitmap: TBitmap;
...
function TFiscalPrinterDriverRR.GetMaxGraphicsHeight: Integer;
begin
  Result := 0;
  if FCapGraphics1 then
    Result := 200;
...
function TFiscalPrinterDriverRR.GetMaxGraphicsWidth: Integer;
begin
  Result := 0;
  if FCapGraphics512 then
  begin
...
function TFiscalPrinterDriverRR.GetMaxGraphicsWidthInBytes: Integer;
begin
  Result := GetMaxGraphicsWidth div 8;
end;

procedure TFiscalPrinterDriverRR.PrintQRCode3(Barcode: TBarcodeRec);
...
procedure TFiscalPrinterDriverRR.PrintBarcodeZInt(const Barcode: TBarcodeRec);
var
  P: TDrawScale;
  Bitmap: TBitmap;
  StartLine: Integer;
...
procedure TFiscalPrinterDriverRR.BarcodeToBitmap(
...
procedure TFiscalPrinterDriverRR.PrintImage(const FileName: WideString;
  StartLine: Integer);
var
  ImageHeight: Integer;
begin
...
procedure TFiscalPrinterDriverRR.PrintImageScale(const FileName: WideString;
...
function TFiscalPrinterDriverRR.GetLineData(Bitmap: TBitmap; Index: Integer): AnsiString;
var
  B: Byte;
  i: Integer;
  Len: Integer;
...
procedure TFiscalPrinterDriverRR.ProgressEvent(Progress: Integer);
begin
  if Assigned(FOnProgress) then
    FOnProgress(Progress);
end;

procedure TFiscalPrinterDriverRR.LoadBitmap(StartLine: Integer; Bitmap: TBitmap);
begin
  Bitmap.Monochrome := True;
  Bitmap.PixelFormat := pf1Bit;
...
procedure TFiscalPrinterDriverRR.LoadBitmap320(StartLine: Integer; Bitmap: TBitmap);
var
  i: Integer;
  Data: AnsiString;
  Count: Integer;
...
procedure TFiscalPrinterDriverRR.LoadBitmap512(StartLine: Integer;
...
function TFiscalPrinterDriverRR.LoadImage(const FileName: WideString;
...
function TFiscalPrinterDriverRR.LoadPicture(Picture: TPicture;
  StartLine: Integer): Integer;
var
  Bitmap: TBitmap;
  XOffset: Integer;
...
function TFiscalPrinterDriverRR.GetFFDVersion: TFFDVersion;
begin
...
function TFiscalPrinterDriverRR.TestCommand(Code: Integer): Boolean;
var
  RxData: AnsiString;
begin
  if Code > $FF then
...
function TFiscalPrinterDriverRR.IsSupported(ResultCode: Integer): Boolean;
begin
  Result := ResultCode <> ERROR_COMMAND_NOT_SUPPORTED;
end;

procedure TFiscalPrinterDriverRR.UpdateInfo;
begin
  GetPrinterModel;
...
function TFiscalPrinterDriverRR.GetTaxCount: Integer;
begin
  Result := Length(FTaxInfo);
end;

// Read font info
function TFiscalPrinterDriverRR.ReadFontInfoList: TFontInfoList;
var
  i: Integer;
  FontInfo: TFontInfo;
begin
...
// Read tax Info
function TFiscalPrinterDriverRR.ReadTaxInfoList: TTaxInfoList;
var
  i: Integer;
...
function TFiscalPrinterDriverRR.IsMobilePrinter: Boolean;
begin
  Result := GetDeviceMetrics.Model = 19;
end;

function TFiscalPrinterDriverRR.GetTaxInfo(Tax: Integer): TTaxInfo;
begin
  Result.Rate := 0;
  Result.Name := '';
...
function TFiscalPrinterDriverRR.ReadCapFiscalStorage: Boolean;
var
  R: TFSState;
begin
  try
...
function TFiscalPrinterDriverRR.WaitForPrinting: TPrinterStatus;
var
  Mode: Byte;
  TryCount: Integer;
  TickCount: Integer;
...
function TFiscalPrinterDriverRR.GetPrinterStatus: TPrinterStatus;
begin
  Result := FPrinterStatus;
end;

function TFiscalPrinterDriverRR.ReadPrinterStatus: TPrinterStatus;
begin
...
function TFiscalPrinterDriverRR.PrintBarLine(Height: Word; Data: AnsiString): Integer;
var
  IsSwapBytes: Boolean;
begin
...
function TFiscalPrinterDriverRR.LoadBarcode2D(const Data: TBarcode2DData): Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
...
function TFiscalPrinterDriverRR.PrintBarcode2D(const Barcode: TBarcode2D): Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
...
function TFiscalPrinterDriverRR.LoadGraphics3(Line: Word; Data: AnsiString): Integer;
begin
...
function TFiscalPrinterDriverRR.LoadGraphics3(const P: TLoadGraphics3): Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
...
function TFiscalPrinterDriverRR.PrintGraphics3(Line1, Line2: Word): Integer;
var
  P: TPrintGraphics3;
begin
  P.FirstLine := Line1;
...
function TFiscalPrinterDriverRR.PrintGraphics3(const P: TPrintGraphics3): Integer;
begin
  DoConnect;
  SetDriverPassword(GetUsrPassword);
  Driver.FirstLineNumber := P.FirstLine;
...
procedure TFiscalPrinterDriverRR.CheckGraphicsSize(Line: Word);
begin

end;

function TFiscalPrinterDriverRR.FilterTLV(Data: AnsiString): AnsiString;
var
  Tag: TTLVTag;
  Item: TTLVItem;
  Tags: TTLVTags;
...
function TFiscalPrinterDriverRR.FSWriteTLV(const TLVData: AnsiString): Integer;
var
  Data: AnsiString;
begin
  Result := 0;
...
function TFiscalPrinterDriverRR.FSSale(P: TFSSale): Integer;
begin
...
function TFiscalPrinterDriverRR.FSSale2(P: TFSSale2): Integer;
const
...
function TFiscalPrinterDriverRR.FSReadRegTag(var R: TFSReadRegTagCommand): Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.FSReadState(var R: TFSState): Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.FSCancelDocument: Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.FSReadStatus(var R: TFSStatus): Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.FSFindDocument(DocNumber: Integer;
...
function TFiscalPrinterDriverRR.FSReadDocMac(var DocMac: Int64): Integer;
var
  FSState: TFSState;
...
function TFiscalPrinterDriverRR.FSReadBlock(const P: TFSBlockRequest;
  var Block: AnsiString): Integer;
begin
  Result := 0;
(*
...
function TFiscalPrinterDriverRR.FSStartWrite(DataSize: Word;
...
function TFiscalPrinterDriverRR.FSWriteBlock(const Block: TFSBlock): Integer;
begin
(*
...
function TFiscalPrinterDriverRR.FSReadBlockData: AnsiString;
var
  i: Integer;
  Count: Integer;
  Block: AnsiString;
...
function TFiscalPrinterDriverRR.GetBlockSize(BlockSize: Integer): Integer;
begin
  Result := BlockSize;
  if Result = 0 then
...
procedure TFiscalPrinterDriverRR.FSWriteBlockData(const BlockData: AnsiString);
begin
  Check(ERROR_COMMAND_NOT_SUPPORTED);
end;

function TFiscalPrinterDriverRR.FSPrintCalcReport(var R: TFSCalcReport): Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.FSReadCommStatus(
...
function TFiscalPrinterDriverRR.FSReadExpiration(var R: TCommandFF03): Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.FSReadFiscalResult(var R: TFSFiscalResult): Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.FSReadTicket(var R: TFSTicket): Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.GetCapFiscalStorage: Boolean;
begin
  Result := FCapFiscalStorage;
end;

function TFiscalPrinterDriverRR.WriteCustomerAddress(const Value: WideString): Integer;
begin
  Result := FSWriteTag(1008, Value);
end;

function TFiscalPrinterDriverRR.FSWriteTag(TagID: Integer; const Data: WideString): Integer;
begin
  Result := FSWriteTLV(TagToStr(TagID, Data));
end;

function TFiscalPrinterDriverRR.ReadFPParameter(ParamId: Integer): WideString;
begin
  case ParamId of
...
procedure TFiscalPrinterDriverRR.WriteFPParameter(ParamId: Integer;
...
function TFiscalPrinterDriverRR.ReadDiscountMode: Integer;
var
  R: TPrinterTableRec;
begin
  Result := 0;
...
function TFiscalPrinterDriverRR.ReadDocPrintMode: Integer;
var
  R: TPrinterTableRec;
begin
  Result := 0;
...
function TFiscalPrinterDriverRR.GetDiscountMode: Integer;
begin
  Result := FDiscountMode;
end;

function TFiscalPrinterDriverRR.GetIsFiscalized: Boolean;
begin
  Result := FIsFiscalized;
end;

function TFiscalPrinterDriverRR.FSReadTotals(var R: TFMTotals): Integer;
begin
...
function TFiscalPrinterDriverRR.FSReadCorrectionTotals(var R: TFMTotals): Integer;
begin
  FillChar(R, SizeOf(R), 0);
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.FSReadTotalsByPayType(RecType: Byte;
...
function TFiscalPrinterDriverRR.ReadDayTotalsByReceiptType(Index: Integer): Int64;
begin
...
function TFiscalPrinterDriverRR.ReadTotalsByReceiptType(Index: Integer): Int64;
begin
  Result :=
...
function TFiscalPrinterDriverRR.ReadDayTotals: TFMTotals;
begin
  Result.SaleTotal := 0;
  Result.BuyTotal := 0;
  Result.RetSale := 0;
...
function TFiscalPrinterDriverRR.ReadFPTotals(Flags: Integer): TFMTotals;
begin
  Result.SaleTotal := 0;
  Result.BuyTotal := 0;
  Result.RetSale := 0;
...
function TFiscalPrinterDriverRR.ReadFPDayTotals(Flags: Integer): TFMTotals;
var
  FPTotals: TFMTotals;
  DayTotals: TFMTotals;
begin
...
function TFiscalPrinterDriverRR.FSPrintCorrectionReceipt(
...
function TFiscalPrinterDriverRR.FSPrintCorrectionReceipt2(
...
procedure TFiscalPrinterDriverRR.LoadTables(const Path: WideString);
var
  i: Integer;
  j: Integer;
  Mask: AnsiString;
...
procedure TFiscalPrinterDriverRR.WriteFields(Table: TPrinterTable);
var
  i: Integer;
  Data: AnsiString;
  Field: TPrinterField;
...
function TFiscalPrinterDriverRR.GetContext: TDriverContext;
begin
  Result := FContext;
end;

function TFiscalPrinterDriverRR.IsFSDocumentOpened: Boolean;
var
  FSState: TFSState;
begin
  Result := False;
...
function TFiscalPrinterDriverRR.IsRecOpened: Boolean;
begin
  Result := (ReadPrinterStatus.Mode and $0F) = MODE_REC;
end;

function TFiscalPrinterDriverRR.ReadLoaderVersion(var Version: WideString): Integer;
var
  Answer: AnsiString;
  Command: AnsiString;
begin
...
function TFiscalPrinterDriverRR.ReceiptClose2(
...
function TFiscalPrinterDriverRR.ReceiptClose22(
  const P: TFSCloseReceiptParams2;
  var R: TFSCloseReceiptResult2): Integer;
begin
...
function TFiscalPrinterDriverRR.ReceiptClose3(
  const P: TFSCloseReceiptParams2;
  var R: TFSCloseReceiptResult2): Integer;
begin
...
function TFiscalPrinterDriverRR.ReadParameters2(
...
function TFiscalPrinterDriverRR.FSFiscalization(const P: TFSFiscalization;
...
function TFiscalPrinterDriverRR.FSReFiscalization(const P: TFSReFiscalization;
  var R: TFDDocument): Integer;
begin
  DoConnect;
  SetDriverPassword(GetSysPassword);
...
function TFiscalPrinterDriverRR.IsCapFooterFlag: Boolean;
begin
  Result := FCapFooterFlag;
end;

procedure TFiscalPrinterDriverRR.SetFooterFlag(Value: Boolean);
begin
  FFooterFlag := Value;
end;

function TFiscalPrinterDriverRR.GetOnPrinterStatus: TNotifyEvent;
begin
  Result := FOnPrinterStatus;
end;

procedure TFiscalPrinterDriverRR.SetOnPrinterStatus(Value: TNotifyEvent);
begin
  FOnPrinterStatus := Value;
end;

procedure TFiscalPrinterDriverRR.SetPrinterStatus(Value: TPrinterStatus);
begin
...
function TFiscalPrinterDriverRR.IsCapBarcode2D: Boolean;
begin
  Result := FCapBarcode2D;
end;

function TFiscalPrinterDriverRR.IsCapEnablePrint: Boolean;
begin
  Result := FCapEnablePrint;
end;

function TFiscalPrinterDriverRR.ReadFSDocument(Number: Integer): WideString;
var
  P: TFSReadDocument;
begin
  Result := '';
...
function TFiscalPrinterDriverRR.ReadDocData: WideString;
var
...
procedure TFiscalPrinterDriverRR.PrintFSDocument(Number: Integer);
begin
  PrintText(PRINTER_STATION_REC, ReadFSDocument(Number));
end;

function TFiscalPrinterDriverRR.FSReadDocument(var P: TFSReadDocument): Integer;
begin
  DoConnect;
...
function TFiscalPrinterDriverRR.FSReadDocData(var P: TFSReadDocData): Integer;
begin
  DoConnect;
  SetDriverPassword(P.Password);
  Result := Driver.FNReadFiscalDocumentTLV;
...
function TFiscalPrinterDriverRR.FSStartOpenDay: Integer;
begin
  DoConnect;
...
procedure TFiscalPrinterDriverRR.EkmCheckBarcode(const Barcode: TGS1Barcode);
var
  Client: TEkmClient;
  SaleEnabled: Boolean;
begin
...
function TFiscalPrinterDriverRR.CheckItemCode(const Barcode: WideString): Integer;
var
...
procedure TFiscalPrinterDriverRR.CheckCorrectItemCode(const P: TFSCheckItemResult);
begin
...
function TFiscalPrinterDriverRR.IsCorrectItemCode(const P: TFSCheckItemResult): Boolean;
begin
  if P.LocalCheckResult = SMFP_LOCAL_CHECK_FAILED then
  begin
    Result := False;
...
function TFiscalPrinterDriverRR.SendItemBarcode(const Barcode: WideString;
  MarkType: Integer): Integer;
var
  Data: AnsiString;
begin
...
function TFiscalPrinterDriverRR.BarcodeTo1162Value(
...
function TFiscalPrinterDriverRR.FSWriteTLVOperation(const AData: AnsiString): Integer;
var
  Data: AnsiString;
begin
  Result := 0;
...
function TFiscalPrinterDriverRR.FSStartCorrectionReceipt: Integer;
begin
  DoConnect;
  SetDriverPassword(FSysPassword);
...
function TFiscalPrinterDriverRR.GetLastDocNumber: Int64;
begin
  Result := FLastDocNumber;
end;

function TFiscalPrinterDriverRR.GetLastDocMac: Int64;
begin
  Result := FLastDocMac;
end;

function TFiscalPrinterDriverRR.GetLastDocTotal: Int64;
begin
  Result := FLastDocTotal;
end;

function TFiscalPrinterDriverRR.GetLastDocDate: TPrinterDate;
begin
  Result := FLastDocDate;
end;

function TFiscalPrinterDriverRR.GetLastDocTime: TPrinterTime;
begin
  Result := FLastDocTime;
end;

function TFiscalPrinterDriverRR.FSReadLastDocNum2: Int64;
var
  FSState: TFSState;
begin
  Result := 0;
...
function TFiscalPrinterDriverRR.FSReadLastDocNum: Int64;
begin
...
function TFiscalPrinterDriverRR.FSReadLastMacValue: Int64;
begin
...
function TFiscalPrinterDriverRR.FSReadLastMacValue2: Int64;
var
  LastMacValue: Int64;
begin
  LastMacValue := 0;
...
function TFiscalPrinterDriverRR.FSCheckItemCode(P: TFSCheckItemCode;
  var R: TFSCheckItemResult): Integer;
begin
...
function TFiscalPrinterDriverRR.FSSyncRegisters: Integer;
begin
  DoConnect;
  SetDriverPassword(FSysPassword);
...
function TFiscalPrinterDriverRR.FSReadMemory(var R: TFSReadMemoryResult): Integer;
begin
  DoConnect;
  SetDriverPassword(FSysPassword);
  Result := Driver.FNGetFreeMemoryResource;
...
function TFiscalPrinterDriverRR.FSWriteTLVFromBuffer: Integer;
begin
  Result := ERROR_COMMAND_NOT_SUPPORTED;
end;

function TFiscalPrinterDriverRR.FSRandomData(var Data: AnsiString): Integer;
begin
  Data := '';
  Result := ERROR_COMMAND_NOT_SUPPORTED;
end;

function TFiscalPrinterDriverRR.FSAuthorize(const DataToAuthorize: AnsiString): Integer;
begin
  Result := ERROR_COMMAND_NOT_SUPPORTED;
end;
...
function TFiscalPrinterDriverRR.FSReadTicketStatus(var R: TFSTicketStatus): Integer;
begin
  DoConnect;
  SetDriverPassword(FUsrPassword);
...
function TFiscalPrinterDriverRR.FSAcceptItemCode(Action: Integer): Integer;
begin
  DoConnect;
  SetDriverPassword(FUsrPassword);
  if Action = 2 then
...
function TFiscalPrinterDriverRR.FSClearMCCheckResults: Integer;
begin
  DoConnect;
  SetDriverPassword(FUsrPassword);
  Result := Driver.FNMarkingClearBuffer;
...
function TFiscalPrinterDriverRR.FSReadMarkStatus(var R: TFSMarkStatus): Integer;
begin
  DoConnect;
  SetDriverPassword(FUsrPassword);
...
function TFiscalPrinterDriverRR.FSStartReadTickets(var R: TFSTicketParams): Integer;
begin
  DoConnect;
  SetDriverPassword(FUsrPassword);
  Result := Driver.FNGetKMServerExchangeStatus;
...
function TFiscalPrinterDriverRR.FSReadNextTicket(var R: TFSTicketData): Integer;
begin
  DoConnect;
  SetDriverPassword(FUsrPassword);
...
function TFiscalPrinterDriverRR.FSConfirmTicket(const P: TFSTicketNumber): Integer;
begin
  DoConnect;
  SetDriverPassword(FUsrPassword);
  Driver.DocumentNumber := P.Number;
...
function TFiscalPrinterDriverRR.FSReadDeviceInfo(var R: string): Integer;
begin
  DoConnect;
  SetDriverPassword(FUsrPassword);
...
function TFiscalPrinterDriverRR.FSReadDocSize(var R: TFSDocSize): Integer;
begin
  DoConnect;
  SetDriverPassword(FUsrPassword);
  Result := Driver.FNGetFreeMemoryResource;
...
procedure TFiscalPrinterDriverRR.STLVBegin(TagID: Integer);
begin
...
procedure TFiscalPrinterDriverRR.STLVAddTag(TagID: Integer;
  TagValue: string);
begin
...
function TFiscalPrinterDriverRR.STLVGetHex: string;
begin
  Result := StrToHexText(FSTLVTag.RawData);
end;

procedure TFiscalPrinterDriverRR.STLVWrite;
begin
  Check(FSWriteTLV(FSTLVTag.RawData));
end;

procedure TFiscalPrinterDriverRR.STLVWriteOp;
begin
  Check(FSWriteTLVOperation(FSTLVTag.RawData));
end;

procedure TFiscalPrinterDriverRR.WriteTLVItems;
var
  i: Integer;
begin
...
procedure TFiscalPrinterDriverRR.FSWriteTLV2(const TLVData: AnsiString);
begin
  FTLVItems.Add(TLVData);
end;

procedure TFiscalPrinterDriverRR.ResetPrinter;
begin
  FTLVItems.Clear;
...
function TFiscalPrinterDriverRR.GetDocPrintMode: Integer;
begin
  Result := FDocPrintMode;
end;

procedure TFiscalPrinterDriverRR.CorrectDate;
var
  PDate: TPrinterDate;
...
procedure TFiscalPrinterDriverRR.CheckPrinterStatus;
begin
  Driver.Check(Driver.ResetECR);
end;

procedure TFiscalPrinterDriverRR.SetCapFiscalStorage(const Value: Boolean);
begin
  FCapFiscalStorage := Value;
end;

function TFiscalPrinterDriverRR.GetTrailerHeight: Integer;
var
  Font: TFontInfo;
begin
...
function TFiscalPrinterDriverRR.GetFont(Font: Integer): TFontInfo;
begin
...
function TFiscalPrinterDriverRR.GetHeaderHeight: Integer;
var
  Font: TFontInfo;
begin
  if FHeadToCutterDistanse <> 0 then
...
function TFiscalPrinterDriverRR.GetTaxInfoList: TTaxInfoList;
begin
  Result := FTaxInfo;
end;

procedure TFiscalPrinterDriverRR.WriteTaxRate(Tax, Rate: Integer);
begin
